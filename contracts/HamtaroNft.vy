...
s_hamtaroTokenUris: DynArray[DynArray[String[64], 100], NB_RARITY]
s_mintFee: uint256
...
def __init__(
    ...
    _hamtaroTokenUris: DynArray[DynArray[String[64], 100], NB_RARITY],
    _mintFee: uint256
    ):
    """
    @notice contructor
    ...
    self.s_hamtaroTokenUris = _hamtaroTokenUris
...
def fulfillRandomWords(_requestId: uint256, _randomWords: DynArray[uint256, NUM_WORDS]):
    ...
    self._setTokenURI(newTokenId, self.s_hamtaroTokenUris[rarity][character])
...
def getRandomCharacterfromRarity(_randomWord: uint256, _indexRarity: uint256) -> uint256:
    ...
    if _indexRarity == 1:
        return _randomWord % 2
    if _indexRarity == 2:
        return _randomWord % 4
    if _indexRarity == 3:
        return _randomWord % 6
    if _indexRarity == 4:
        return _randomWord % 12
    raise "Out of range"
...
@external
@view
def getHamtaroTokenUri(_rarity: uint256, _index: uint256) -> String[64]:
    """
    ...
    """
    return self.s_hamtaroTokenUris[_rarity][_index]
